		...
		if(args['restore_np']):
			...
			if (args['attack'] == 'EN'):
				attack = EADEN(sess, model, batch_size=args['batch_size'], max_iterations=args['maxiter'], confidence=args['conf'], initial_const=args['init_const'], 
					binary_search_steps=args['binary_steps'], targeted = not args['untargeted'], beta=args['beta'],lam=args['lam'], abort_early=args['abort_early'])
				adv = attack.attack(inputs, targets)
		...
		for m,model in enumerate(model_):
			...
			for i in range(0,len(inputs),num_targets):
				...
				if success_average:				
					...
					d_average_psnr.append(-10*np.log10(np.sum((adv[rand_int]-inputs[rand_int])**2)/(32.0*32.0)))
					d_average_l0.append(np.count_nonzero(adv[rand_int]-inputs[rand_int]))
			...
			if(num_targets > 1):
				...
				print(lbl+'average_case_PSNR_mean', np.mean(d_average_psnr))
				print(lbl+'average_case_L0_mean', np.mean(d_average_l0))
				...
				print(lbl+'PSNR_mean', np.mean(d_average_psnr))
				print(lbl+'L0_mean', np.mean(d_average_l0))
				print(lbl+'success_prob', np.mean(r_average))		
				with open('log.txt','a') as f:
					f.write(str(args['numimg'])+' '+str(args['lam'])+' '+str(args['beta'])+'\n')
					f.write(lbl+'L1_mean '+str(np.mean(d_average_l1))+'\n')
					f.write(lbl+'L2_mean '+str(np.mean(d_average_l2))+'\n')
					f.write(lbl+'Linf_mean '+str(np.mean(d_average_linf))+'\n')
					f.write(lbl+'PSNR_mean '+str(np.mean(d_average_psnr))+'\n')
					f.write(lbl+'L0_mean '+str(np.mean(d_average_l0))+'\n')
					f.write(lbl+'success_prob '+str(np.mean(r_average))+'\n')
...
if __name__ == "__main__":
	...
	parser.add_argument("-la", "--lam", type=float, default=1, help='lambda hyperparameter')
